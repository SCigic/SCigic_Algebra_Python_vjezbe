...
def open_account():
    ...
    if amount != '':
        ...
        transaction.append(str(datetime.datetime.now().date()))
        transaction.append(str(datetime.datetime.now().time()))
...
def display_account_balance():
    ...
    print(f'Datum i vrijeme:\t{datetime.datetime.now().date()} {datetime.datetime.now().time()}\n')
...
def display_account_transactions():
    ...
    for k,v in transactions.items():
        print(f"Transaction {k}: {v[5]}: {v[2]}. Transakcija je izvršena na dan {v[0]}. Ukupni iznos na računu: {v[3]}")
    print()
    input('Za Povratak u Glavni izbornik pritisnite bilo koju tipku\t')
...
def account_deposit():
    ...
    deposit = float(input("Koliko novaca želite položiti na račun: "))
    transaction = []
    ...
    transaction.append(account_balance)
    transaction.append(account_number)
    transaction.append('Novi depozit')
    transaction.append(company_manager)
    transactions[transaction_id + 1] = transaction
    transaction_id += 1
...
def account_withdrawal(): 
    ...
    withdrawal = float(input("Koliko novaca želite skinuti s računa: "))
    transaction = []
    ...
    transaction.append(account_balance)
    transaction.append(account_number)
    transaction.append('Povlačenje sredstava')
    transaction.append(company_manager)
    transactions[transaction_id + 1] = transaction
    transaction_id += 1
